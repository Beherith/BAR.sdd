...
local minMajorEngineVersion = 104
...
function Warning()
    ...
    if string.find((Engine and Engine.version),".",1,true) then 
        local n = string.find((Engine and Engine.version),".",1,true)
        reportedMajorVersion = string.sub((Engine and Engine.version),1,n-1)   
        local m = string.find((Engine and Engine.version),"-",1,true)
        local m2 = string.find((Engine and Engine.version),"-",m+1,true)
        reportedMinorVersion = string.sub((Engine and Engine.version),m+1,m2-1)
        devEngine = true
    else 
        local n = string.len((Engine and Engine.version))
        reportedMajorVersion = string.sub((Engine and Engine.version),1,n)  
        devEngine = false
    end
    ...
    then
        Spring.Echo(red .. "WARNING: You are using Spring " .. (Engine and Engine.version) .. ", which is too old for this game.")
        Spring.Echo(red .. "Please update your engine to  " .. wantedEngineVersions)
    elseif reportedMajorVersion>maxEngineVersion then
        Spring.Echo(red .. "WARNING: You are using Spring " .. (Engine and Engine.version).. " which is too recent for this game.")
